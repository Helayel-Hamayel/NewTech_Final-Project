  ...
  return (
    <section className="resident-view" aria-labelledby="dashboard-heading">
      ...
      <div className="resident-summary-grid">
        <section className="resident-card resident-balance-card" aria-labelledby="balance-heading">
          <div className="resident-balance-header">
            <p className="section-label section-label--light">Account balance due</p>
            <h2 className="resident-balance-total">{currency.format(totalBalance)}</h2>
          </div>
          ...
          <div className="resident-card-actions">
            <button className="resident-primary-btn" type="button">Pay All · {currency.format(totalBalance)}</button>
            <button className="resident-secondary-btn resident-secondary-btn--light" type="button">Payment Plan</button>
          </div>
        </section>
        ...
        <article className="resident-card resident-kpi-card resident-kpi-card--rose">
          ...
          <button type="button" onClick={() => onNavigate('My Tickets')}>View in My Tickets</button>
        </article>
      ...
      <section className="resident-card resident-status-card" aria-labelledby="status-heading">
      ...
      <section className="resident-card resident-utility-card" aria-labelledby="utility-heading">
        <div className="resident-card-header resident-card-header--stacked">
          ...
          <div className="resident-utility-toggle" aria-label="Utility type selection">
            <button type="button" className={utilityType === 'Water' ? 'is-active' : ''} onClick={() => onUtilityTypeChange('Water')}>Water</button>
            <button type="button" className={utilityType === 'Electricity' ? 'is-active' : ''} onClick={() => onUtilityTypeChange('Electricity')}>Electricity</button>
          </div>
        </div>
        ...
        <figure className="resident-chart">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={utilityUsage}><CartesianGrid strokeDasharray="3 3" stroke="#dbe3ef" /><XAxis dataKey="month" tick={{ fill: '#64748b', fontSize: 11 }} stroke="#cbd5e1" /><YAxis tick={{ fill: '#64748b', fontSize: 11 }} stroke="#cbd5e1" /><Tooltip
              contentStyle={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '12px', color: '#0f172a' }}
              cursor={{ fill: '#dbeafe' }}
            /><Bar dataKey="usage" name={utilityType} radius={[6, 6, 0, 0]} fill="#2563eb" /></BarChart>
          </ResponsiveContainer>
        </figure>
        ...
        <aside className="resident-current-bill">
          ...
          <button className="resident-secondary-btn" type="button">View billing history</button>
        </aside>
      </section>
      ...
      <section className="resident-card" aria-labelledby="fines-heading">
        ...
        {unpaidFines.length === 0 ? <p className="resident-empty-state">No unpaid citations.</p> : <ul className="resident-list resident-list--stacked">{unpaidFines.map((fine) => (
          <li key={fine.id}>
            <article className="resident-list-item">
              <div className="resident-list-meta">
                ...
                <div>
                  ...
                  <p className="resident-muted resident-list-location">{fine.location}</p>
                </div>
              </div>
              <div className="resident-list-details">
                <p><span>Status</span> <strong className="status-badge status-badge--unpaid">{fine.status}</strong></p>
                <p><span>Location</span> {fine.location}</p>
                <p><span>Date</span> {fine.date}</p>
                <p><span>Amount</span> {currency.format(fine.amount)}</p>
              </div>
              <button className="resident-secondary-btn" type="button" onClick={() => onAppeal(fine.id)}>File Appeal</button>
            </article>
          </li>
        ))}</ul>}
      </section>
